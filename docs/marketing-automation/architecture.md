...
The sandbox receives only a normalized delivery record: delivery ID, repository, branch, merge commit SHA, actor ID, pull-request number, and a derived GitHub files URL. It never receives the raw body, signature, or webhook secret. Missing policy or secret configuration rejects the request. The host replay guard bounds and suppresses duplicate dispatches within one runtime, while deterministic storage run IDs make retries converge. `StorageCollection.create` provides an atomic insert-if-absent claim across runtimes; the sync attempt protocol uses those claims for idempotency-key reservations and lease generations, then persists bounded terminal projections for replay. This supports one durable attempt per claim, while the external apply operation still depends on its revision checks and content/media idempotency semantics.
